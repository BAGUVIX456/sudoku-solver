...
impl Sudoku {
    ...
    pub fn get_table(&mut self) -> Result<(), anyhow::Error> {
        let mut file_path = String::new();

        // This is to be changed to parse command line arguments using clap
        println!("\nEnter location of XLSX file:");
        io::stdin()
        ...
        let mut workbook: Xlsx<_> =
            open_workbook(file_path.trim()).context("Failed to get xlsx file")?;
        let mut return_val = Vec::new();

        // Not happy with the unwrap method here, will try to replace it with something more reliable
        ...
        // replace all the println!s with buffered writeln!
